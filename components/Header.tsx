...
export default function Header() {
  ...
  return (
    <header className="relative z-10 px-6 md:px-12 pt-10 pb-8 border-b border-ink-700/70">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <p className="mono text-[10px] uppercase tracking-[0.3em] text-bone-400 mb-2 dot-pulse">
            <span className="text-gold">●</span> Active protocol · day {elapsed + 1}
          </p>
          ...
          <p className="mt-3 text-sm text-bone-300 max-w-md font-light">
            ...
            {PROTOCOL.startWeight} kg → {PROTOCOL.targetWeight} kg by {new Date(PROTOCOL.endDate).toLocaleDateString("en-GB", { day: "numeric", month: "long" })}.
          </p>
        </div>

        <div className="flex items-center gap-8">
          <div className="text-right">
            <p className="mono text-[10px] uppercase tracking-[0.25em] text-bone-400">Days left</p>
            <p className="serif text-5xl text-bone-50 font-light">{remaining}<span className="text-bone-400 text-2xl">d</span></p>
          </div>

          <svg width="80" height="80" viewBox="0 0 80 80" className="rotate-[-90deg]">
            <circle cx="40" cy="40" r="34" fill="none" stroke="#dccfbe" strokeWidth="2" />
            <circle
              cx="40" cy="40" r="34" fill="none"
              stroke="#c98b96" strokeWidth="2"
              strokeLinecap="round"
